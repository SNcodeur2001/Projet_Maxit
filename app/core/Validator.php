...
class Validator
{
    ...
    private static function initRules()
    {
        if (empty(self::$rules)) {
            self::$rules = [
                ...
                'file_required' => function($value, $key, $message) {
                    return isset($_FILES[$key]) && $_FILES[$key]['error'] === UPLOAD_ERR_OK ? true : $message;
                },
                'file_image' => function($value, $key, $message) {
                    if (!isset($_FILES[$key]) || $_FILES[$key]['error'] !== UPLOAD_ERR_OK) return true;
                    $allowedTypes = ['image/jpeg', 'image/jpg', 'image/png'];
                    return in_array($_FILES[$key]['type'], $allowedTypes) ? true : $message;
                },
                'file_max_size' => function($value, $key, $message, $params = []) {
                    if (!isset($_FILES[$key]) || $_FILES[$key]['error'] !== UPLOAD_ERR_OK) return true;
                    $maxSize = $params['size'] ?? (5 * 1024 * 1024);
                    return $_FILES[$key]['size'] <= $maxSize ? true : $message;
                },
    ...
    private function applyRuleWithMessage($field, $value, $rule, $message, $data)
    {
        ...
        return self::$rules[$ruleName]($value, $field, $message, $params, $data) ?? true;
    }
    ...
    private function applyRule($field, $value, $rule, $messages, $data)
    {
        ...
        $message = $messages[$messageKey] ?? $messages[$field] ?? $defaultMessage;
    ...
    private function getDefaultMessage($field, $rule, $params = [])
    {
        ...
        $defaultMessages = [
            ...
            'min_length' => "Le champ {$fieldName} doit contenir au moins " . ($params['length'] ?? 3) . " caractères.",
            'max_length' => "Le champ {$fieldName} ne peut pas dépasser " . ($params['length'] ?? 255) . " caractères.",
            ...
            'file_max_size' => "Le fichier {$fieldName} ne peut pas dépasser " . $this->formatFileSize($params['size'] ?? (5 * 1024 * 1024)) . ".",
            'accepted' => "Vous devez accepter {$fieldName}.",
            'numeric' => "Le champ {$fieldName} doit être numérique."
        ];

        return $defaultMessages[$rule] ?? "Le champ {$fieldName} n'est pas valide.";
    }
